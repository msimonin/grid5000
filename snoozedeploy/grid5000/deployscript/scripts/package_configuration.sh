...
create_and_deploy_centralized_group_manager_configurations () {
    ...
    echo "$log_tag listen on $start_control_data_port"
    echo "$log_tag monitoring on $start_monitoring_data_port"
...
create_and_deploy_distributed_group_manager_configurations () {
    ...
    for group_manager_address in $(cat "$tmp_directory/group_managers.txt"); do
        echo "CREATION OF GM"
        echo "$log_tag Configuring group manager with parameters ""$virtual_machine_subnet, $start_control_data_port, $start_monitoring_data_port, $group_manager_heartbeat_mcast_port, $zookeeper_addresses"
